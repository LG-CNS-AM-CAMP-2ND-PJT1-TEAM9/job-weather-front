...
const JobSearch = () => {
  ...
  const applyDummyFilters = (
    ...
    console.log(
      `더미필터 적용됨: term=${term}, loc=${location}, exp=${experience}, type=${employmentType}`
    );
    return jobs.filter((job) => {
      ...
      const employmentTypeMatch =
        employmentType === "전체" ||
        (job.title && job.title.includes(employmentType));

      return (
        termMatch && locationMatch && experienceMatch && employmentTypeMatch
      );
    });
  };
    ...
    const fetchJobAndBookmarkData = async () => {
      ...
      if (currentUserSn) {
        try {
          ...
          console.log(
            `현재 사용자 ${currentUserSn} 의 북마크를 가져오는 중:`,
            bookmarkListApiUrl
          );

          const bookmarkResponse = await fetch(bookmarkListApiUrl, {
            credentials: "include",
          });
          ...
          if (!bookmarkResponse.ok) {
            ...
            bookmarkedIds = Array.isArray(bookmarkData)
              ? bookmarkData.map(String)
              : [];
        ...
        console.log(
          "사용자가 로그인하지 않았습니다. 즐겨찾기 목록을 가져오지 않습니다."
        );
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `작업 중 http 오류 발생: ${apiUrl} 에서 ${response.status} 발생 `
          );
        }
        ...
        if (parsedJobs.length === 0) {
          console.log(
            "API에서 작업 0개를 반환했으므로 더미 데이터에서 필터를 적용합니다."
          );
          const filteredDummyJobs = applyDummyFilters(
            dummyJobs,
            searchTerm,
            selectedLocation,
            selectedExperience,
          ...
          const filteredAndBookmarkedDummyJobs = filteredDummyJobs.map(
            (job) => ({
              ...job,
        ...
        console.log(
          "API 작업 데이터를 사용할 수 없으므로 더미 데이터 필터를 적용"
        );
  ...
  const handleBookmarkClick = async (jobId, isCurrentlyBookmarked) => {
    ...
    if (isCurrentlyBookmarked) {
      console.log(
        `북마크 작업이 건너뛰어졌습니다. ${jobId}은 이미 북마크되었습니다.`
      );
      return;
    }
    ...
    if (!isValidSaraminId) {
      console.warn(
        `북마크 작업이 건너뛰어졌습니다. 백엔드에 대한 작업 ID 형식이 잘못되었습니다 :  ${jobId}`
      );
    ...
    try {
      console.log(
        `Attempting to add bookmark for job ${jobId} by user ${currentUserSn}`
      );
      ...
      if (!response.ok) {
        ...
        console.error(
          `북마크 추가 API 호출 실패: ${response.status}`,
          errorDetail
        );
    ...
    <>
      <div className={styles["job-container"]}>
        ...
        <div className={styles["job-list"]}>
          ...
          {!isLoadingAuth && !loading && error && jobList.length === 0 && (
